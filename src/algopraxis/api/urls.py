...
urlpatterns = [
    ...
    url(r'^codeset/(?P<slug>[\w-]+)/create_or_update/(?P<lang_mode>[\w-]+)$', views.CodeSetCreateUpdateAPIView.as_view(), name='codeset-create-update'),
    # Solution's views
    url(r'^solution/(?P<id>\d+)/$', views.SolutionDetailAPIView.as_view(), name='solution-detail'),
    url(r'^solution/(?P<slug>[\w-]+)/create_or_update/(?P<lang_mode>[\w-]+)$', views.SolutionCreateUpdateAPIView.as_view(), name='solution-create-update'),
